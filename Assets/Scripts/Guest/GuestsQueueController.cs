...
public class GuestsQueueController : MonoBehaviour
{
    ...
    [SerializeField] private GuestFaces[] _facesSprites = new GuestFaces[5];

    void Start()
    {
        _queue = new Guest[]{ 
            new Guest(
                AchievementIndex.Lemonade,
        ...
        _policeController = _guest.GetComponent<PoliceController>();
    ...
    public void ToNextGuest() {
        ...
        else if (_currentGuestNumber == _queue.Length) {
            _policeController.CreateDialog();
            _currentGuestNumber++;
        }
    }
}
